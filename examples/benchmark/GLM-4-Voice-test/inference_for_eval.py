...
def main(args):
    ...
    # do inference 
    with jsonlines.open(pred_text, mode="w") as pt, jsonlines.open(gt_text, mode="w") as gt, jsonlines.open(question_text, mode="w") as qt:
        for step, s in enumerate(tqdm(data_list, total=len(data_list), desc=f"evaluating on {args.val_data_name}")): 
            if args.manifest_format == "datasets": 
                audio_raw, sr, source_text = s['audio']['array'], s['audio']['sampling_rate'], s['prompt']
                if 'reference' in s:
                    target_text = s['reference']
                else:
                    target_text = s['prompt']
            elif args.manifest_format == "jsonl": 
                source_wav = os.path.join(os.path.dirname(args.val_data_path), s['source_wav'])
                source_text = s['source_text']
                if "target_text" in s.keys(): 
                    target_text = s['target_text'] 
                else: target_text = s['source_text']
                audio_raw, sr = librosa.load(source_wav)
            audio_raw = torch.from_numpy(audio_raw).unsqueeze(0)
            user_input = process_audio((audio_raw, sr), whisper_model, feature_extractor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manifest_format", type=str, choices=["datasets", "jsonl"], 
