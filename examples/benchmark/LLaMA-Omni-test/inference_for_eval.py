...
def eval_model(args, questions):
    ...
    for (input_ids, speech_tensor, speech_length), item in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        results.append({"question_id": idx, "prediction": outputs, "prediction_units": output_units if args.s2s else None, 
                        "question": item['question'], "target": item['target']})
    return results

def generate_waveform(args, prediction_units, id):
...
def main():
    ...
    with open(args.dataset, 'r') as f:
        data_length = sum(1 for _ in jsonlines.Reader(f))
    with open(args.dataset, 'r') as f:
    ...
    # inference
    logging.info(f"<=========inference starts=========>")
    os.makedirs(args.results_path, exist_ok=True)
    output_audio_path = os.path.join(args.results_path, 'audio')
    os.makedirs(output_audio_path, exist_ok=True)
    pred_text = os.path.join(args.results_path, "pred_text")
